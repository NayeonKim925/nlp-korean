...
**한 줄 요약: 한국어 혐오/공격 표현 탐지 모델이 문맥이 아니라 집단 지칭 단어 자체에 반응하는 shortcut 문제를, "타당성이 검증된" counterfactual 쌍에만 일관성 제약을 거는 방식(validity-gated CCR)으로 완화했습니다. 핵심 발견 — identity 치환은 label-preserving 여부를 검증한 뒤에만 정규화 신호로 써야 robustness가 개선됩니다. 정규화 강도(λ_eff)를 동일하게 맞춘 상태에서도 이 효과는 유지되며, 그 결과 23% 적은 쌍(5,964개)만으로 최고 robustness(StrictPairAcc 0.8295, FlipRate 0.0205)를 Macro-F1 손실 없이 달성했습니다.**
...
1. **Gate가 고른 쌍의 품질이 정규화 총량보다 중요합니다** — Strict-Matched는 λ_eff를 Naive Swap과 동일하게 맞췄는데도(즉 정규화 강도는 같음) PairAcc·StrictPairAcc 모두 최고를 기록했습니다 (Naive Swap 대비 각각 +0.0096 / +0.0124). 이걸 **23% 적은 쌍(5,964 vs 7,735)**으로 달성했다는 것은, 쌍의 양이 아니라 gate가 걸러낸 쌍의 질이 핵심 동인임을 보여줍니다. StrictPairAcc 궤적: Baseline 0.8076 → Naive 0.8171 → **Strict-Matched 0.8295**, bootstrap 검정 기준 유의함 (PairAcc p=0.0002, StrictPairAcc p<0.001).
