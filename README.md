...
**Coverage-aware λ control (Strict-Matched).** gate를 적용하면 valid pair의 coverage가 감소하므로, Strict-Matched는 coverage 감소를 보정하기 위해 λ를 0.1297로 증가시켰다. 여기서 사용하는 `λ_eff`는 coverage를 요약하기 위한 **heuristic 지표**이며, gradient 수준의 정규화 압력을 정확히 등화하는 것은 아니다. 따라서 품질 효과의 1차 비교는 **동일 λ=0.1의 Naive Swap과 Strict-Gated**에서 수행하고, Strict-Matched는 coverage 감소에 대한 heuristic control 조건으로 보고한다.
...
모든 조건은 동일한 base 모델 셋업을 사용하며, 차이는 **어떤 counterfactual 신호를 사용하는가**뿐이다.
...
동일 λ=0.1의 **Naive Swap vs Strict-Gated**는 gate 자체의 효과를 비교하며, **Strict-Matched**는 coverage 감소를 heuristic하게 보정한 대표 조건이다.
...
1. **Validity of counterfactual pairs matters more than quantity.** 동일 λ=0.1에서 Strict-Gated는 Naive Swap보다 PairAcc와 StrictPairAcc가 모두 높다. 또한 coverage를 보정한 Strict-Matched는 최고 성능을 기록한다. 즉 성능 향상의 핵심은 더 많은 pair가 아니라 더 타당한 pair이다. 엄밀한 통계적 유의성 검정은 수행하지 않았으며, 이 결론은 3-seed 평균과 λ sweep 전반의 일관된 경향에 기반한다.

2. **Robustness improves without sacrificing Macro-F1.** 모든 조건의 Macro-F1은 0.788~0.792 범위를 유지하여 robustness 향상이 분류 성능 저하를 수반하지 않았다.

3. **Semantic counterfactuals outperform masking.** Masking 기반 regularization은 제한적인 개선만 보였으며, 의미가 보존된 identity swap을 사용하는 CCR이 가장 효과적이었다.
...
- **λ_eff = λ × c 보정은 heuristic이다.** gradient 수준의 정규화 압력을 정확히 등화하는 것은 아니며, per-example weighting 등이 향후 과제이다.
- **단일 데이터셋과 단일 encoder family, 3개 seed**에 기반한 결과이다. paired bootstrap 등의 엄밀한 유의성 검정과 다양한 모델로의 확장이 필요하다.
- **Rule-based gate**이므로 미묘한 pragmatic validity 실패와 OOD 일반화에는 한계가 있다.
